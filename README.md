...
- **무선 카플레이(CarPlay)를 쓰는 중에는 미러링이 안 돼요.** 아이폰은 무선 카플레이와 화면 미러링을 **동시에 할 수 없어요**(아이폰 자체 제약이라 앱으로는 못 풀어요). 미러링하려면 카플레이를 끄고, 아이폰을 이 기기의 Wi-Fi(또는 핫스팟)에 연결한 뒤 **화면 미러링 → MirrorDrive**를 선택하세요.
